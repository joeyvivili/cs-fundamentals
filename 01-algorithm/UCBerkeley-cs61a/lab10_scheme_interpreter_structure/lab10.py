...
def floor_div(args):
    ...
    "*** YOUR CODE HERE ***"
    if args.rest is nil:
        raise TypeError("There must be at least two elements for floor division")
...
def math_apply(op, args, initial):
    while args is not nil:
        initial = op(initial, args.first)
        args = args.rest
    return initial

def addition(args):
    return math_apply(operator.add, args.rest, args.first)

def subtraction(args):
    if args.rest is nil:
        return -args.first
    else:
        return math_apply(operator.sub, args.rest, args.first)

def multiplication(args):
    return math_apply(operator.mul, args.rest, args.first)

def division(args):
    if args.rest is nil:
        return 1 / args.first
    else:
        return math_apply(operator.truediv, args.rest, args.first)

OPERATORS = { "//": floor_div, "+": addition, "-": subtraction, "*": multiplication, "/": division }
# OPERATORS = { "//": floor_div, "+": operator.add, "-": operator.sub, "*": operator.mul, "/": operator.truediv }
