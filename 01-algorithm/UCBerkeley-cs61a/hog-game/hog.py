...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    strategy0: The strategy for player0. A strategy is a function that, 
                given a player's score and their opponent's score, returns the number of dice 
                that the current player will roll in that turn. 
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_average_roll, max_average = 0, 0
    roll = make_averaged(roll_dice, times_called)
    for i in range (1, 11):
        average = roll(i, dice)
