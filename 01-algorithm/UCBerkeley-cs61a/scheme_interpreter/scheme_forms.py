...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not (isinstance(binding, Pair) and scheme_listp(binding)):
    ...
    # Reverse the lists to maintain order
    reversed_names = nil
    while names is not nil:
        reversed_names = Pair(names.first, reversed_names)
        names = names.rest
    reversed_vals = nil
    while vals is not nil:
        reversed_vals = Pair(vals.first, reversed_vals)
        vals = vals.rest
    # END PROBLEM 14
    return env.make_child_frame(reversed_names, reversed_vals)
