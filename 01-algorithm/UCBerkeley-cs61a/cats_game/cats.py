...
def minimum_mewtations(typed, source, limit):
    ...
    # If first characters match, no edit needed
    if typed[0] == source[0]:
        return minimum_mewtations(typed[1:], source[1:], limit)
    else: # Try all three edits
...
def final_diff(typed, source, limit):
    ...
    # incorporate in real typing mistakes
    # Base cases
    if limit < 0:
        return float('inf')
    if typed == source:
    ...
    if len(typed) > 1 and typed[0] == typed[1]:
        return final_diff(typed[1:], source, limit - 0.5)  # smaller penalty

    # Recursive calls for add, remove, replace
    add_diff = 1 + final_diff(typed, source[1:], limit - 1)
    remove_diff = 1 + final_diff(typed[1:], source, limit - 1)
    replace_diff = (typed[0] != source[0]) + final_diff(typed[1:], source[1:], limit - 1)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress_ratio = correct_count / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        times.append([timestamps_per_player[i][j+1] - timestamps_per_player[i][j] for j in range(len(words))])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    player_fastest_words = [[] for i in range(len(player_indices))]
    """ Logic: for each word in words list, get each player's time for that word, find the minimun time among players and the respective player, then append that word to that player; return the list of each player's fastest words at the end """
    for word in word_indices:
        player_word_time = [time(match, player_num, word) for player_num in player_indices]
        fastest_time = min(player_word_time)
        fastest_player_index = player_word_time.index(fastest_time)
        player_fastest_words[fastest_player_index].append(get_word(match,word))
