...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'is_doubled') == False:
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while True:
            if p.is_hive:
                    return None
            if place_index > self.upper_bound:  # subclass ShortThrower has a upper_bound attack range constraint
                    return None
            if self.lower_bound <= place_index: # subclass LongThrower has a lower_bound attack range constraint
                if p.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place == None:  # in the case if FireAnt dies after damange
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance = 0
        insect_and_distance = {}
        while p is not None and p.is_hive == False:
            for bee in p.bees:
                insect_and_distance[bee] = distance
            if p.ant is not None:
                if p.ant is not self:
                    insect_and_distance[p.ant] = distance
                if p.ant.is_container == True:
                    if p.ant.ant_contained is not None:
                        if p.ant.ant_contained is not self:
                            insect_and_distance[p.ant.ant_contained] = distance
            p = p.entrance
            distance += 1
